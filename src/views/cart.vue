              ...
              <li v-for="(item,index) in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascript:;" class="checkbox-btn item-check-btn" :class="{'check': item.checked == 1}" @click="cartEdit('select',item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click.prevent="cartEdit('minus', item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click.prevent="cartEdit('add', item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{ (parseInt(item.productNum) * parseInt(item.salePrice)) | currency('￥')}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="comfirmDel(item,index)">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis': checkCount === 0}" @click.prevent="GoAddress">Checkout</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartList.forEach(item => {
        if(item.checked === 1) {
          sum += parseFloat(item.productNum) * parseFloat(item.salePrice);
        }
      });
      return sum;
    }
    ...
    Del() {
      axios.post('/users/cartDel',{
        ...
        if(res.status == '0'){
          ...
          this.$store.commit('updateCartCount',-1 * this.delItem.productNum);
          this.closeModal();
        }
      })
    },
