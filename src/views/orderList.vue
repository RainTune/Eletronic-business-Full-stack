<template>
  <div>
    ...
    <nav-content>
      <slot slot="bread">orderList</slot>
    </nav-content>
      ...
      <div class="checkout-order">
            ...
            <ul class="cart-item-list">
              <li v-for="item in buyList">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{(parseFloat(item.productNum) * parseFloat(item.salePrice)) | currency('￥')}}</div>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <router-link class="btn btn-warning" to="/address">Previous</router-link>
          </div>
    ...
    methods: {
      getBuyList() {
        axios.get('/users/cartList').then(response => {
          ...
          if(res.status === '1') {
            /*过滤选中购物车的数据*/
            this.buyList = res.result.filter(item => {
              return item.checked == 1;
            });
            this.buyList.forEach(item => {
              this.subtotal += parseFloat(item.salePrice) * parseFloat(item.productNum);
            })
          }
        })
      },
